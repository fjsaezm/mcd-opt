...
Let $S$ be an open set. Clearly, \(cl(S^c) \subset S^c\).

Now, let \(x\) be an element of \(S^c\). Then,
\[
\forall \delta > 0, \quad B(x,\delta) \cap S^c \neq \emptyset
\]

\(\boxed{\Leftarrow}\)\\

Let us see that \(S^c = cl(S^c)\) since, if this happens, \(S\) is open.\\

Let \(x\in S\). Then, \(x \notin S^c\), which implies \(x \notin cl(S^c)\). This implies that
\[
\exists \lambda > 0 \text{ such that } B(x,\lambda) \cap S^c = \emptyset \implies B(x,\delta) \subset S \implies S \text{ is open}.  
\]
...
x'' - z'' \in S_1 - S_2   
\]
so the segment is in the difference set, so the set \(S_1 - S_2\) is convex.
\end{itemize}
